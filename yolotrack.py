...
video_result = cv2.VideoWriter('mut_results.avi',  
                         cv2.VideoWriter_fourcc(*'MJPG'), 
                         22, (1024, 768)) 

cap = cv2.VideoCapture("./test_videos/pizza_test.mp4")
...
while(cap.isOpened()):
	...
	if np.any(crossed_in):
		seconds_elapsed = int(frame_count/fps.fps())
		time_change = datetime.timedelta(seconds=seconds_elapsed) 

		new_date = initial_time + time_change
		delivered.append(["returned", new_date])
		total_in += 1
	...
	if np.any(crossed_out):
		...
		new_date = initial_time + time_change
		delivered.append(["delivered", new_date])
		total_out += 1
...
df = pd.DataFrame(delivered, columns=["delivered_event", "delivered_at"])
df.to_csv("delivered_results.csv", index=False)
